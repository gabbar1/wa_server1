...
const WAZIPER = {
	...
	makeWASocket: async function (instance_id) {
		...
		await WA.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr, receivedPendingNotifications }) => {
			console.log("============connection.updateeeee=========");
			console.log(qr);
		...
		await WA.ev.on('creds.update', async (c)=>{
	...
	//const { connectToDatabase } = require('./db.js'); // Adjust the path as needed

	//const { connectToDatabase } = require('./db.js'); // Adjust the path as needed

	instance: async function (access_token, instance_id,user_id, login, res, callback) {
		...
		try {
			...
			// const team = await database.collection('sp_team').findOne({ ids: access_token.toString });
			// console.log("===========team===============>");
		    // console.log(team);
			// if (!team) {
			// 	if (res) {
			...
			const session = await database.collection('sp_whatsapp_sessions').findOne({
				user_id: parseInt(user_id),
				instance_id:instance_id


				//team_id: team._id // Assuming team_id is stored as an ObjectId
			});

			if (!session) {
				// Update account status in MongoDB
				await database.collection('sp_accounts').updateOne(
					{ token: instance_id },
					{ $set: { status: 0 } }
				);
	...
	button_template_handler: async function (template_id, params) {
		console.log("=============button_template_handler=============>");
	...
	list_message_template_handler: async function (template_id, params) {
		console.log("============list_message_template_handler==============>");
...
cron.schedule('*/2 * * * * *', function () {
	//console.log("============cron_live_back==============>");
	WAZIPER.live_back();
});

cron.schedule('*/1 * * * * *', function () {
	//console.log("==========cron_bulk_messaging================>");
